...
#ifdef STUDENT
    int status;
    char *jobcommand = strdup(jobcmd(j));
    int jobstatus = jobstate(j, &status);
    if (which == ALL){
      if (jobstatus == FINISHED) {
        if WIFEXITED(status)
          msg("[%d] exited '%s', status=%d\n", j, jobcommand, WEXITSTATUS(status));
        else
          msg("[%d] killed '%s' by signal %d\n", j, jobcommand, WTERMSIG(status));
      }
